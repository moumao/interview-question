...
let theDiv = null, move = false;//拖拽对象 拖拽状态
let theDivId =0, theDivHeight = 0, theDivHalf = 0; tarFirstY = 0; //拖拽对象的索引、高度、的初始化。
...
function loopbox(){ //循环初始化
    $(".drag_module_box").find(".drag_module_main").each(function(){
        $(this).mousedown(function (event){
            ...
            $("<li class='drag_module_dash'></li>").insertBefore(theDiv);
        });
    });
    ...
    // 拖拽元素随鼠标移动
    theDiv.css({left: lastPos.x + 'px',top: lastPos.y -150+ 'px'});
    // 拖拽元素随鼠标移动 查找插入目标元素
    let $main = $('.drag_module_main'); // 局部变量：按照重新排列过的顺序 再次获取 各个元素的坐标，
